...
eg：https://raw.githubusercontent.com/Keywos/rule/main/rename.js#name=测试&flag
-------------------------------- 
...
[name=]:  添加机场名前缀在节点最前面
[out=]:   输出节点名可选参数: (zh ，enshort ，flag ，enfull) 对应：(中文，英文缩写 ，国旗 ，英文全称) 默认中文
--------------------------------
以下为不常用参数:
[in=]:    自动判断机场节点名类型(那种类型多就判断为那种)(优先匹配原节点国旗) 也可以加参数指定 (zh ，enshort ，flag ，enfull)
[nx]:     保留1倍率与不显示倍率的
[blnx]:   只保留高倍率
[clear]:  清理乱名
*/
...
const inname =  iar.in === "zh" ? "zh" :  iar.in === "enshort" ? "enshort" :  iar.in === "enfull" ? "enfull" :  iar.gq === "flag" ? "flag" : "";
function gl(arg) { switch (arg) { case "flag": return flag; case "enshort": return enshort; case "enfull": return enfull; default: return zh; }}
...
function gReg(pn) { if (flag.some((name) => pn.includes(name))) { return "flag"; } else if (zh.some((name) => pn.includes(name))) { return "zh"; } else if (enfull.some((name) => pn.includes(name))) { return "enfull"; } else if (enshort.some((name) => pn.includes(name))) { return "enshort"; } else { return null; } } 
